...
/**
 * Deploys the GatekeeperOne contract from Ethernaut level 13
 *
...
const deployGatekeeperOne: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log("Deploying GatekeeperOne contract with account:", deployer);
  ...
  console.log(`GatekeeperOne contract deployed at: ${gatekeeperOne.address}`);
  ...
  if (!isLocalNetwork(chainId) && process.env.ETHERSCAN_API_KEY) {
    ...
    try {
      ...
      if (error.message.includes("Already Verified")) {
        ...
        console.log("You can manually verify later using: npx hardhat run scripts/verify.ts --network sepolia -- --address", gatekeeperOne.address);
      } else {
...
deployGatekeeperOne.tags = ["level-13", "gatekeeper-one"];
