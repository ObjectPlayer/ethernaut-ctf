...
/**
 * Deploys the Force contract from Ethernaut level 7
 *
...
const deployForce: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log("Deploying Force contract with account:", deployer);

  // Deploy the Force contract
  ...
  if (!isLocalNetwork(chainId) && process.env.ETHERSCAN_API_KEY) {
    ...
    try {
      ...
      if (error.message.includes("Already Verified")) {
        ...
        console.log("You can manually verify later using: npx hardhat run scripts/verify.ts --network sepolia -- --address", force.address);
      } else {
...
deployForce.tags = ["level-07", "force"];
